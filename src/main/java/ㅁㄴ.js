{name:'os',index:'os', width:90 , sorttype:function(cell, obj){

    return obj.hwosname+"("+obj.hwosversion+")";
} , align:"center", editable:false ,formatter : function(cellV , op , rowObj){
...
{name:'domainnm' ,index:'domainnm', width:90 , sortype:'text' , align:"left", editable:false ,formatter :function(cellV,op,rowObj){
...
{name:'fir' ,index:'fir', width:90 , sortype:'text' , align:"left", editable:false, formatter : function(cellV , op , rowObj){
...
{name:'sec',index:'sec', width:90 , sortype:'text' , align:"left", editable:false,formatter : function(cellV , op , rowObj){
...
<div id="searchOSDialog" title='<s:text name="AGENT.AGENT_REGISTRY.LABEL.SEARCH_OS"/>'>
    <div id="searchOSDialog_Wrap">
    //새로 작성이 필요
//     <div id="searchOSFind">
...
E:\workspace_NEW\workspace_190516\SecuMS_branch\src\main\webapp\WEB-INF\pages\server\system\hwsystemDetail.jsp
<th>OS</th>
<tD id="hwgorup_wrap">

    </tD>
    <td id="hwmaster_seq_wrap">
    </td>
...
List hwgroup = ssmHwDao.ssmCodeList(map);
List hwmasterList = ssmHwDao.ssmhwmasterList(map);


<table width="100%" class="sm-table-search">
    <colgroup>
    <col width="20"/>
    <col width="20"/>
    <col width="50"/>
    ...
    <tr>
    <th>OS</th>
    <tD id="hwgorup_wrap"> </tD>
    <td id="hwmaster_seq_wrap"> </td>
    </tr>
    </tbody>
    </table>
    ...
    <select name="POL_MNGT_SEQ" id="POL_MNGT_SEQ">
    <c:forEach var="list" items="${listPolicyMasterList}">
    <option value="${list.POL_MNGT_SEQ}"
    <c:if test="${list.REP_YN eq 'Y'}">selected</c:if>>${list.POL_MNGT_NM}</option>
</c:forEach>
</select>
...
<th>OS</th>
<td id="hwgorup_wrap">
    <select id="hwgroup" name="hwgroup" onchange="PG.fn.hwmasterChg(this.value)" style="width:200px">

    </select>
    <%--<select>--%>
    <%--<option value="">시스템그룹 선택</option>--%>
<%--<option value="02">Unix</option>--%>
    <%--<option value="01">Windows</option>--%>
    <%--<option value="05">Linux</option>--%>
    <%--<option value="06">Windows-PC</option>--%>
    <%--</select>--%>
    </td>
    </tr>
    <tr>
...
var radioB = $("<label>" +
    "<"input type = 'radio' id = '" + PG.data.hwmaster[count].hwgroup + "' name = '" + PG.data.hwmaster[count].hwgroup + "' />
